...
import { compareNames, find, getNameAttribute, identity, isPublic, } from '@openscd/open-scd/src/foundation.js';
...
export default class CompareIEDPlugin extends DirectDialogMixin {
    ...
    render() {
        ...
        if (this.selectedProjectIed && this.selectedTemplateIed) {
            return html `${this.renderDialog(this.renderCompare(), get('compare-ied.resultTitle'))}${this.renderWizardDialog()}`;
        }
        else if (this.templateDoc) {
            return html `${this.renderDialog(this.renderIEDLists(), get('compare-ied.selectIedTitle'))}${this.renderWizardDialog()}`;
        }
        else {
            return html `${this.renderDialog(this.renderSelectTemplateFile(), get('compare-ied.selectProjectTitle'))}${this.renderWizardDialog()}`;
        }
    }
}
